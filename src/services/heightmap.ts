...
class Heightmap {
	...
	degreesLongAtLatToMeters(degrees: number, latDegrees: number): number {
		...
		return (
			((degrees * CIRCUMFERENCE_MARS_METERS) / 360) *
			Math.abs(Math.cos(latRadians))
		);
	}
	...
	metersToDegreesLongAtLat(meters: number, latDegrees: number): number {
		...
		return (
			(meters * 360) /
			CIRCUMFERENCE_MARS_METERS /
			Math.abs(Math.cos(latRadians))
		);
	}

	async getPointsFromMySql(
		startLat: number,
		endLat: number,
		startLong: number,
		endLong: number
        ...
        WHERE
          ...
          AND longitude BETWEEN ${escape(startLong)} AND ${escape(
				endLong
			)};`;
	...
	async generate(params: any): Promise<IHeightmap> {
		...
		console.log(params);
		console.log(
			`CenterLat ${centerLat}, centerLong ${centerLong}, distance ${distance}`
		);
		...
		const startLong =
			centerLong + this.metersToDegreesLongAtLat(distance, centerLat);
		const endLong =
			centerLong - this.metersToDegreesLongAtLat(distance, centerLat);

		const points = await this.getPointsFromMySql(
			startLat,
			endLat,
			startLong,
			endLong
